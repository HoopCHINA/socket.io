...
/**
 *   search(array, value, [insert: Boolean = false]): Integer
 *     */
function _bsearch(o, v, i) {
...
Manager.prototype.onJoin = function (id, name) {
  ...
  var index = _bsearch(this.rooms[name], id, true);

  if (this.rooms[name][index] != id) {
    this.rooms[name].splice(index, 0, id);
...
Manager.prototype.onLeave = function (id, room) {
  if (this.rooms[room]) {
    var index = _bsearch(this.rooms[room], id);
